...
def get_price(ticker):
    """
    Narxni olish - bir nechta endpoint sinab ko'radi.
    1. Snapshot (to'lovli)
    2. Prev day aggs (bepul)
    3. Yahoo Finance (backup)
    """
    # 1. Snapshot (to'lovli Polygon)
    try:
        r = requests.get(
        ...
        if d:
            ...
            if price and price > 0:
                prev_c = prev.get("c", 0)
                chg = ((price - prev_c) / prev_c * 100) if prev_c > 0 else d.get("todaysChangePerc", 0)
                return {
                    "price":  price,
                    "open":   day.get("o", 0), "high": day.get("h", 0),
                    "low":    day.get("l", 0), "close": day.get("c", 0),
                    "volume": day.get("v", 0), "vwap":  day.get("vw", 0),
                    "prev":   prev_c, "chg": round(chg, 2),
                }
    except Exception:
        pass

    # 2. Prev day aggs (bepul Polygon)
    try:
        r = requests.get(
            "https://api.polygon.io/v2/aggs/ticker/" + ticker +
            "/prev?adjusted=true&apiKey=" + POLYGON_KEY, timeout=10)
        results = r.json().get("results", [])
        if results:
            d = results[0]
            o_ = d.get("o", 0); c_ = d.get("c", 0)
            chg = ((c_ - o_) / o_ * 100) if o_ > 0 else 0
            return {
                "price":  c_,
                "open":   o_, "high": d.get("h", 0),
                "low":    d.get("l", 0), "close": c_,
                "volume": d.get("v", 0), "vwap":  d.get("vw", 0),
                "prev":   o_, "chg": round(chg, 2),
            }
    except Exception:
        pass
    ...
    try:
        ...
        logging.warning("get_price backup xato (" + ticker + "): " + str(e))
...
def get_candles(ticker, days=90):
    ...
    try:
        r = requests.get(
            ...
            "?adjusted=true&sort=asc&limit=100&apiKey=" + POLYGON_KEY, timeout=15)
